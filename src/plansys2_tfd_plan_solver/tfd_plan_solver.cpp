...
void TFDPlanSolver::configure(rclcpp_lifecycle::LifecycleNode::SharedPtr lc_node,
                              const std::string& plugin_name)
{
  ...
  {
    RCLCPP_FATAL(lc_node_->get_logger(), "'TFD_HOME' environment variable not defined for %s",
                 plugin_name.c_str());
    exit(-1);
  }
                       ...
                       const std::string& node_namespace, const rclcpp::Duration solver_timeout)
{
  ...
  status =
      system(("mv " + output_sas_file_path.string() + " " + output_sas_file_path.string() + ".last")
                 .c_str());
  if (status == -1)
  {
  ...
  status = system(("mv " + processed_output_file_path.string() + " " +
                   processed_output_file_path.string() + ".last")
                      .c_str());
  if (status == -1)
  {
  ...
  status =
      system(("mv " + pddlplan_file_path.string() + ".1 " + pddlplan_file_path.string() + ".1.last")
                 .c_str());
  if (status == -1)
  {
...
bool TFDPlanSolver::isDomainValid(const std::string& domain, const std::string& node_namespace)
{
  if (system(nullptr) == 0)
  {
    return false;
